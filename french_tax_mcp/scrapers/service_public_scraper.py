...
from french_tax_mcp.constants import (
    SERVICE_PUBLIC_BASE_URL as BASE_URL,
    SERVICE_PUBLIC_PARTICULIERS_URL as PARTICULIERS_URL,
    SERVICE_PUBLIC_TAX_SECTION_URL as TAX_SECTION_URL,
    SERVICE_PUBLIC_INCOME_TAX_URL as INCOME_TAX_URL,
    SERVICE_PUBLIC_PROPERTY_TAX_URL as PROPERTY_TAX_URL,
    SERVICE_PUBLIC_DEADLINES_URL as DEADLINES_URL,
    FRENCH_MONTHS
)
...
class ServicePublicScraper(BaseScraper):
    ...
    def _extract_steps(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        if step_elements:
            ...
            for heading in soup.find_all(["h2", "h3"]):
                ...
                if re.match(r"^\d+\.", heading_text) or any(word in heading_text.lower() for word in ["étape", "phase", "partie"]):
                    content = []
    ...
    def _extract_related_procedures(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for heading in soup.find_all(["h2", "h3", "h4"]):
            ...
            if any(keyword in heading_text for keyword in ["voir aussi", "à voir également", "en lien", "liens utiles"]):
