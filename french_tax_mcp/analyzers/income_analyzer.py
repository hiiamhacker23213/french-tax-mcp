...
class IncomeTaxAnalyzer:
    ...
    async def calculate_income_tax(
        ...
        logger.info(f"Calculating income tax for {net_taxable_income}€ with {household_parts} parts for year {tax_year}")
    ...
    async def calculate_household_parts(
        ...
        """
        logger.info(f"Calculating household parts for {marital_status} with {num_children} children and {disabled_dependents} disabled dependents")
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to calculate household parts: {str(e)}"
            }
