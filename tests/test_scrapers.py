...
class TestImpotsScraper:
    ...
    async def test_get_scheme_details(self, mock_scheme_response):
        ...
        scraper._get_scheme_url = MagicMock(return_value="/particulier/questions/comment-beneficier-du-dispositif-pinel")
        
        # Call get_scheme_details
        result = await scraper.get_scheme_details("pinel", 2023)
        
        # Check that get_page was called with the correct URL
        scraper.get_page.assert_called_once_with("/particulier/questions/comment-beneficier-du-dispositif-pinel")
...
class TestServicePublicScraper:
    ...
    async def test_get_tax_procedure(self):
        ...
        # Mock the get_page method
        scraper.get_page = AsyncMock(return_value=MagicMock(text="<html><body><h1>Test</h1></body></html>"))
...
class TestLegalScraper:
    ...
    async def test_search_tax_law(self):
        ...
        # Mock the get_page method
        scraper.get_page = AsyncMock(return_value=MagicMock(text="<html><body><div class='search-result'><h3>Test</h3></div></body></html>"))
